...
fn adjust_down() {
    fn run_with(mut next: impl FnMut(&mut dyn Sound) -> Result<NextSample, RawedioError>) {
        ...
        assert_eq!(next(&mut first).unwrap(), NextSample::Sample(500));
    }
...
fn adjust_up() {
    fn run_with(mut next: impl FnMut(&mut dyn Sound) -> Result<NextSample, RawedioError>) {
        ...
        assert_eq!(next(&mut first).unwrap(), NextSample::Sample(5000));
    }
...
fn test_saturation() {
    fn run_with(mut next: impl FnMut(&mut dyn Sound) -> Result<NextSample, RawedioError>) {
        ...
        assert_eq!(next(&mut first).unwrap(), NextSample::Sample(i16::MAX));
    }
